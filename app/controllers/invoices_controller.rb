class InvoicesController < ApplicationController
  ...
  def add_kosongan
    ...
    @have_kosongan = Invoice.active.where(kosongan_previous_invoice_id: @invoice_id).where("id NOT IN (SELECT invoice_id FROM invoicereturns WHERE deleted = 'f')")
...
#    @gascost = Setting.find_by_name("Harga Solar").value.to_i rescue nil || 0
  ...
  def updatetrain
    ...
    @invoice.operator_id = inputs[:operator_id]
    @invoice.routetrain_id = inputs[:routetrain_id]

    if @invoice.save
      redirect_to("/invoices/add_train/"+@invoice.id.to_s)
    end
  ...
  def indexaddtrain
    ...
    @invoices = @invoices.where("invoicetrain = true and invoice_id is null and invoices.id not in (select invoice_id from trainexpenses where deleted = false)")
  ...
  def get_routes
    ...
    if is_train == "0"
      ...
      if params[:kosongan] == 'produktif' || params[:kosongan] == 'non-produktif'
        ...
        @routes = Route.active.where(customer_id: cust_kosongan).where("approved = true").order(:name)
        ...
        @routes = Route.where(:customer_id => params[:customer_id], :enabled => true, :deleted => false, :approved => true).where("name !~* '.*depo.*'").order(:name)
      end
  ...
  def get_routesbyoffice
    ...
    if is_train == "0"
        ...
        @routes = Route.where(:office_id => params[:office_id], :enabled => true, :deleted => false, :approved => true).order(:name)
  ...
  def get_routeswithtype
    ...
    @routes = Route.where(:customer_id => params[:customer_id], :enabled => true, :deleted => false, :approved => true).order(:name)
