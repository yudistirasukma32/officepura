class InvoicereturnsController < ApplicationController
  ...
  def index
    ...
    @invoices = Invoice.where('date = ? AND deleted = false AND invoicetrain = false AND id IN (SELECT invoice_id FROM receipts WHERE deleted = false)', @date.to_date).order(:id) 
  ...
  def index_confirmed
    @date = params[:date]
    @date = Date.today.strftime('%d-%m-%Y') if @date.nil?
    @invoices = Invoice.where('date = ? AND deleted = false AND invoicetrain = false', @date.to_date).order(:id) 
  ...
  def create
    ...
    if inputs[:process] == "create"
      ...
      if inputs[:quantity].to_i > @invoice.quantity
        ...
        @invoicereturn.total = @invoicereturn.driver_allowance.to_i + @invoicereturn.helper_allowance.to_i + @invoicereturn.misc_allowance.to_i + @invoicereturn.tol_fee.to_i + @invoicereturn.ferry_fee.to_i + @invoicereturn.premi_allowance.to_i
  ...
  def destroy
    ...
    if @invoicereturn.receiptreturns.where(:deleted => false).any?
      ...
      @invoicereturn.save
      redirect_to(invoicereturns_url)
    end
  end  
