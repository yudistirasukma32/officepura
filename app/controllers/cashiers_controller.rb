class CashiersController < ApplicationController
  ...
  def set_section
    ...
    @date = Date.today.strftime('%d-%m-%Y') if @date.nil? 
		...
		@officeexpenses_operational = Officeexpense.find_by_sql("SELECT o.* FROM officeexpenses o INNER JOIN officeexpensegroups g ON o.officeexpensegroup_id = g.id where o.date = '#{@date.to_date}' AND o.deleted = false AND g.officeexpensegroup_id = #{Officeexpensegroup.find_by_name('UMUM OPERASIONAL (NOPOL)').id} AND o.id NOT IN (SELECT officeexpense_id FROM receiptexpenses where deleted = false)")
