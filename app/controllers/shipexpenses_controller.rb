class ShipexpensesController < ApplicationController
  ...
  def index
    ...
    @shipexpenses = Invoice.where('routeship_id is not null AND routeship_id !=0 AND date between ? and ? AND invoices.id not in (select invoice_id from shipexpenses where deleted = false)', @startdate.to_date, @enddate.to_date).where("id not in (select invoice_id from invoicereturns where deleted = false)").order(:id)
