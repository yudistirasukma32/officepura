...
$(document).ready(function() {
	...
	$(document).on('click', '#calculateDistance', function(e) {
		...
		if (!longitudeStart || !latitudeStart || !longitudeEnd || !latitudeEnd) {
			...
			var url = `/routelocations/checkDistance?latitude_start=${latitudeStart}&longitude_start=${longitudeStart}&latitude_end=${latitudeEnd}&longitude_end=${longitudeEnd}`;
		...
		if (!longitudeStart || !latitudeStart || !longitudeEnd || !latitudeEnd) {
			...
			$.ajax({
				...
				success: function(data) {
					alert('Estimasi jarak: ' + data.distance_km + ' km');
					console.log(data.distance_km + ' km');
					// $("#tabbed li").removeClass("current").first().addClass("current");
					// $(".sliding_container").css("left", "0");

					// $("#route_distance").val(data.distance_km).focus();
